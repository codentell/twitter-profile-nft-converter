    ...
    {session ? (
      <>
        <Section>
          ...
          <p style={{color: 'white'}}>{session.user && `https://twitter-profile-nft-converter-backend-kfrs.vercel.app/${session.user.name ? session.user.name : 'placeholder'}?theme=${theme}&profile=${session.user.image?.replace("_normal", "")}`}</p>
          <section style={{display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr 1fr", gridGap: "10px"}}>
